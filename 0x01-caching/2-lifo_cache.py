...
class LIFOCache(BaseCaching):
    """
    LIFOCache class. It is a caching system with a Last-In, First-Out
    algorithm.
    ...
    def put(self, key, item):
        ...
        If the key or item is None or an empty string, this method should
        not do anything.
        If the number of items in self.cache_data is higher than
        BaseCaching.MAX_ITEMS,
        it must discard the last item put in cache (LIFO algorithm).
        """
        if not key or not item:
            return
        if (
            key not in self.cache_data and len(
                self.cache_data
            ) >= BaseCaching.MAX_ITEMS
        ):
